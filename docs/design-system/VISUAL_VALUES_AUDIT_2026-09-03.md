...
> ⚠️ **Это снимок состояния на 03.09.2026, а не действующее описание.** Значения
> в нём — «как было до Дизайн-системы v1.0». Действующие правила — Документ А в
> Notion, `src/styles/tokens.css` и `.claude/skills/hexlash-design`.
>
> В частности, **всё, что этот файл говорит про знак, устарело.** На 03.09 в коде
> жил предыдущий знак с тремя отрисовками; 05.09.2026 знак заменён, отрисовок две,
> порог один — 48 точек. Действующее описание —
> `docs/design-handoff/hexlash_mark/README.md`.
...
| `src/components/brand/hexlashMark.js` | 67 | знак ⚠️ *(состояние 03.09; знак заменён 05.09, см. шапку)* |
...
`src/components/brand/hexlashMark.js` ⚠️ *(снимок 03.09 — знак с тех пор заменён, отрисовок две, порог 48; см. шапку)*: `markVariantFor(size)` → `size >= 64 ? 'full' : size >= 24 ? 'compact' : 'micro'`. viewBox соответственно `0 0 64 64` / `0 0 48 48` / `0 0 16 16`.
...
`hexlashMark.js` ⚠️ *(снимок 03.09 — см. шапку)* определял `markVariantFor(size)`: `≥64 → full`, `24–48 → compact`, `<24 → micro`. Комментарий в `home.css` предупреждает, что коробка не должна пересекать 24 и 48 без смены отрисовки.
