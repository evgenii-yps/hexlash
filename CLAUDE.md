...
### Klich → shared ability model (per-card uses + global anti-spam cooldown)

Replaced the klich's single shared pool (`klichPool` 3-per-bout) with the ability model that will be reused by future buffs. Only `ArenaScene.vue` + `KlichBar.vue` touched — the klich effect (`applyKlich` axis bump), the arm→highlight→tap→pulse loop, and `klichProfiles.js` are unchanged.

- **Per-card uses** — `klichUses` ref `{ forward: 2, retreat: 2, hold: 2 }` (`KLICH_USES = 2`, tunable). Applying a card spends **that card's own** counter (−1); the corner badge shows the card's remaining uses. A card at 0 is **spent** (darkened, dead until the match resets). Cancel-arming spends nothing.
- **Global anti-spam cooldown** — `KLICH_COOLDOWN_MS = 6000` (tunable, one place). After **any** application, `startCooldown()` sets `cooldownActive` + a `setTimeout`; while active all cards are **paused** (inactive, can't arm) but **not** spent; on expiry cards with uses left re-enable. A SHARED resource (not klich-hardcoded) — `startCooldown`/`clearCooldown`/`cooldownActive` are generic, reusable by future buffs. Timer-driven logic (applies under reduced-motion too; no per-frame tick — the ref flip drives re-render).
- **Three distinct card states** in `KlichBar.vue` (props `uses` + `cooldown`): available (opacity 1.0) / paused (0.55 + cool neutral border, temporary) / spent (0.28, permanent). `:disabled` on `uses<=0 || cooldown`; `armLever` guards both.
- **Reset on new bout** — `resetKlich()` (called by `runFight` + `runSigFight`) restores per-card uses, clears the cooldown timer + flag, and un-arms. `cooldownTimer` also cleared in `onBeforeUnmount`.

Build green; reduced-motion intact (paused/spent are static opacity; only the armed pulse animates, already gated). Protected arena files untouched.
