...
взаимный риск (правка деки задевает игру, правка игры задевает деку).

Сейчас там **каркас** — фон, знак, заголовок, одна строка подписи, подвал.
Содержимого деки нет: это шаг 2, отдельное ТЗ.

| Файл | Роль |
|---|---|
| `public/deckinvestors/index.html` | вся страница целиком: разметка, стили, мета |
| `vercel.json` | исключение из перехвата всех адресов |

**Адрес держится двумя вещами, и обе легко снести нечаянно:**

1. Форма «папка со своим `index.html`». Её одинаково понимают и Vercel, и nginx
   (`try_files $uri $uri/` в `nginx.prod.conf`). Файл вида `deckinvestors.html`
   работал бы только на одном из двух путей.
2. Исключение в `vercel.json`: там стоит перехват **всех** адресов
...
**Закрыта от поиска** мета-запретом внутри страницы плюс заголовком ответа
`X-Robots-Tag` в `vercel.json`. В `robots.txt` путь **не упоминается
намеренно**: строки запрета в том файле публичны и работают как указатель.
Ссылок на страницу нет ниоткуда — ни из меню, ни из подвала, ни из карты сайта.

**Превью ссылки** у страницы своё (заголовок, описание, `og-image.png` 1200×630).
Запрет индексации ему не мешает: первое закрывает от поиска, второе работает
при отправке адреса руками.

**Значения вписаны литералами** — так же и по той же причине, что на экране
загрузки (`#hx-load` в `index.html`): страница живёт вне сборки, `tokens.css`
туда не приезжает, `var()` без объявления не работает. Источник правды прежний —
`src/styles/tokens.css`; меняется там, потом здесь.

**Знак** — те же файлы `public/brand/*`, что у остальных поверхностей, а не
отдельная копия картинки. Разметка написана руками, потому что
`src/components/brand/hexlashMark.js` — Vue-модуль и на эту страницу не грузится
(ровно так же поступает экран загрузки). Бокс знака 128 точек на большом экране
и 59 на телефоне 360 — оба **выше порога 48**, значит везде полная отрисовка.
⚠️ Опустить бокс ниже 48 — показать другой рисунок знака.

**Известное расхождение.** Заголовок `X-Robots-Tag` добавлен только на пути
Vercel. `nginx.prod.conf` (путь Docker + K8s из `.github/workflows/gitops.yaml`)
не тронут: какой из двух путей отдаёт прод, из репозитория однозначно не
следует, а проверить конфиг nginx здесь нечем. Мета-запрет внутри страницы
работает на обоих путях, поэтому от поиска она закрыта в любом случае.
