...
## Новый знак Hexlash — раскатан на все поверхности

Знак заменён везде одной правкой. Эталон — `docs/design-handoff/hexlash_mark/`
(README = геометрия и правила, INTEGRATION.md = как подключено и где разошлись).

**До раскатки в проде было два разных логотипа одновременно:** в игре и на лендинге
один рисунок (сегментированный гекс с росчерком, растр `logo-512.png`), в иконке
вкладки и приложения — совсем другой (гекс с Y-росчерком, обводками). Причина: знак
в приложении когда-то заменили, а зашитые координаты в `scripts/gen-favicons.mjs` —
нет. Плюс og-картинка была третьим вариантом. Раскатка это свела.

### Одна точка входа

`src/components/brand/hexlashMark.js` — три отрисовки под свои размеры и правило
переключения `>=64 полная · 24–48 упрощённая · <=20 мелкая`. Второй копии геометрии
в проекте нет. Знак — `fill="currentColor"` без своего цвета: красится через `color`
у родителя, поэтому светлый фон = смена цвета, а не второй файл (инверсный SVG в код
не подключён — он та же геометрия с другим `color`).

Мелкая отрисовка выше 16px **не масштабируется, а дополняется полем** (расширяется
`viewBox`) — она нарисована по пикселям, растяжение убило бы весь её смысл.

### Поверхности (8)

Панель игры 40px · загрузка (обе половины) · навигация и подвал лендинга ·
404 · авторизация · сброс пароля · иконки вкладки и приложения · og-картинка.

Экран загрузки, 404 и авторизация знак **получили** — раньше там был только текст
(вопреки комментариям в коде, которые обещали «mark + wordmark»). Композиция —
вертикальный лок-ап, знак сверху, слово снизу; пропорции сняты пиксельным замером
с `hexlash-lockup-vertical.svg`, связка собрана в вёрстке через одну переменную
`--word`, чтобы масштабироваться как целое.

⚠️ **Бокс знака нельзя менять, не проверив границы 24 и 48** — отрисовка должна
меняться вместе с ним.

### Иконки

`node scripts/sync-brand-icons.mjs` — раскладывает готовые рендеры хэндофа в
`public/` и пересобирает `favicon.ico` (16px мелкая + 32px упрощённая). Скрипт
ничего не рисует, только копирует: предыдущий `gen-favicons.mjs` рисовал знак сам
по зашитым координатам, и именно та копия разъехалась. Он удалён.

Манифест получил иконку под круглую обрезку (`icon-maskable-512.png`) — её не было.

### Удалено

- `src/assets/images/logo-512.png` — старый растровый знак, ноль потребителей
- `scripts/gen-favicons.mjs` — генератор старого знака
- `LogoMark` из `components/landing/icons.js` — знак переехал в свой модуль

### Правило бренда не изменилось

Монохром `#F6F4F6`, без свечения, без градиентов, розового в знаке нет.
