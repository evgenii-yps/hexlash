...
**Правило бренда изменено владельцем.** Было «монохром, розового в знаке нет
никогда» — стало «белый плюс единственный акцент, фирменный розовый ровно на
одном росчерке». Свечения, градиентов и объёма в знаке по-прежнему нет; охранное
поле — не меньше четверти высоты знака. Логотип как был вне дизайн-системы, так и
остался: правило «один розовый на экране» на него не распространяется, потому что
розовый здесь — часть знака, а не акцент интерфейса. Переписаны оба документа —
`.claude/skills/hexlash-design` и этот файл.
...
Знак **несёт свои цвета в файле** и больше не наследует `color`. Прежний был
монохромным `fill="currentColor"`; все семь объявлений `color`, стоявших только
...
меняться вместе с ним. Пометка стоит у каждого места подключения.
...
- **Мелкий бокс на плотном экране.** Самый крупный файл упрощённой отрисовки —
  64px, а панель игры (40px) и подвал лендинга (44px) на телефоне с тройной
  плотностью просят 120–132px. Знак там чуть мягче остального. Лечится одним
  файлом `mark-micro-128.png` в пакете, в коде менять нечего.
- **Знак на экране загрузки грузится файлом**, а не вписан в разметку (прежний
  монохромный путь красился мгновенно). Взамен стоит `<link rel="preload">`.
...
## Первый знак Hexlash (01.09.2026) — историческая справка

⚠️ Раздел описывает **прежний** знак («боец в гексе», монохром), выведенный из
обращения 05.09.2026. Оставлен ради причины, по которой существуют нынешние
правила.

**До той раскатки в проде было два разных логотипа одновременно:** в игре и на
лендинге один рисунок (сегментированный гекс с росчерком, растр `logo-512.png`),
в иконке вкладки и приложения — совсем другой. Причина: знак в приложении когда-то
заменили, а зашитые координаты в `scripts/gen-favicons.mjs` — нет. Плюс
og-картинка была третьим вариантом.

Отсюда два действующих правила: **одна точка входа** (`hexlashMark.js`) и **скрипт
иконок только копирует, никогда не рисует**. Оба пережили смену знака.

Тот знак имел **три** отрисовки с порогами 64 и 24, был монохромным
`fill="currentColor"` и наследовал цвет от родителя. Мелкая отрисовка была нарисована
по пикселям на сетке 16×16 и выше 16px не масштабировалась, а дополнялась полем.
Ничего из этого к нынешнему знаку не относится.
...
одной переменной `--word`) сохранена и на новом знаке.
