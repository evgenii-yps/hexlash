...
| `/` | MarketingView — лендинг, секции по компонентам в `src/components/landing/` (переписан PR #408) / redirect to `/play` (authed via beforeEnter) | Public |
| `/help` | PageView | Yes |
...
| Marketing (лендинг) | `MarketingView.vue` + `src/components/landing/*` | ⚠️ **Переписан PR #408** — прежнее описание (единый файл ~915 строк с восемью секциями внутри) устарело; секции 8b/8c ниже описывают страницу, которой больше нет. Сейчас: `MarketingView.vue` — оболочка на ~150 строк, секции вынесены в компоненты `LandingNav / LandingHero / LandingCode / LandingGameplay / LandingToken / LandingRoadmap / LandingStayUpdated / LandingFooter` плюс фон `LandingBackground` + `HexGrid` (canvas). Стили — один файл `landing.css`, всё под пространством имён `.lp`. **Первый экран (05.09.2026):** заголовок `HEXLASH` одной строкой, `clamp(76px, 23vw, 330px)`; свечение вынесено отдельным слоем `.glow` (копия слова с прозрачными буквами, несёт только тень) и мерцает через `lpk-glowflick` — 13 с, два события с неровными паузами ~5 и ~8 с, по два толчка, дно 0.4, петля только при `prefers-reduced-motion: no-preference`. Ниже два уровня текста (`.lead` / `.lead-sub`). Кнопки PLAY / JOIN DISCORD; ⚠️ ширина PLAY держится боковым отступом `78px`, подобранным под измеренную ширину DISCORD — жёсткой ширины нет намеренно. Стрелка прокрутки скрыта ≤680px. **Шапка — только знак**, слово HEXLASH снято со всех ширин 05.09.2026 (оно дублировало заголовок); высота шапки выведена из размера знака: `--nav-h = знак + 40px`, а первый экран считает `min-height: 100vh − --nav-h`, поэтому размер знака двигает и высоту шапки, и положение первого экрана. **Своя крупная шкала заголовков** (решение Р1, `tokens.css` §0) — общая шкала кеглей на витрину не распространяется; цвет, линии и движение берутся из токенов. **Своё правило `prefers-reduced-motion`**, которое анимации не выключает, а проигрывает один раз — новые петли обязаны гаситься собственным `no-preference`. Текст лендинга зашит в разметку, кроме раздела `landing.hero.*` в `en.js` (заведён 05.09.2026, первый на лендинге). Authed users redirect to `/play` via beforeEnter. Мета — `useDocumentMeta`; ⚠️ описание в `index.html` и подставляемое во время работы расходятся. |
...
> **RULE: Claude Code may merge to `main` — but only on explicit owner confirmation in chat for that specific merge.** Never merge or push `main` automatically or on Claude's own initiative. Default for a finished task: commit + push to the **working branch only**, then stop. When the owner explicitly approves a given merge in chat, Claude Code may merge that branch into `main` (fast-forward where possible) and push. **Before merging, record the rollback point** (the current `main` commit) so a revert is one command away. After the push, report the resulting `main` HEAD + the recorded rollback commit. Each merge needs its own confirmation — approval of one merge does not authorize the next.
>
...
> ⚠️ **Историческая запись. Страницы, описанной ниже, больше нет.** Лендинг переписан
> PR #408: единый `MarketingView.vue` заменён оболочкой с секциями-компонентами в
> `src/components/landing/`. Текущее состояние — в строке «Marketing (лендинг)»
> таблицы Key Views. То же относится к секции 8c ниже.
...
### Reading the foe's action phase — сбив замаха + контра (read skill)

Closes the corner hole from the 19.06 audit: micro-states existed on a fighter (`windupVulnUntil`, `staggerUntil`, recovery) but weren't exposed to the foe, so сбив happened only by random timing coincidence and CATCH (Засада) only defended. Now a fighter READS the foe's action phase as a SKILL (delayed + error-prone, scaled by the counter axis) and consciously punishes it. Spinal cord only (model OFF default). Protected arena files untouched; reduced-motion path intact (the read subsystem lives after the reduced-motion early-return — under reduced the foe runs no attack clips, so phase is mostly neutral and the reflex simply doesn't fire).

**Phase exposed.** `buildFighter.getActionPhase()` → `'windup' | 'commit' | 'recovery' | 'stagger' | 'neutral'` (early interruptible swing / committed pre-impact / post-last-impact open tail / interrupt-lock / nothing). Pure read of existing state, no cost. Wired foe→foe in `ArenaScene.vue` via `getFoePhase` (mirrors the existing `getFoeReacting`/`getFoeStamina` pattern, both directions).

**Read is noised by counter (навык, не данность).** In `buildFighter` an `updateRead(t)` keeps a PERCEIVED phase that (a) lags the true phase by `readDelaySec(counter01)` and (b) may MISS a transition entirely (`readMissChance(counter01)`, perception stays stale). Low counter → slow + blind; high counter → fast + sharp, but NEVER perfect (`missChanceHigh = 0.12 > 0` = reliability cap < 1, so a maxed ambusher is still hittable and the bout never locks into eternal сбив). A rare ложное чтение (`readFalseChance`) makes it lunge at a phantom opening (whiff). All numbers in `combatBalance.read` + the `read*` helper exports (one tuning point, counter-scaled functions).

**Read → conscious reaction** (`tryReadReaction`, called in the AI tick before `decideAttack`; NOT gated by `intentionFlags.attack`, so CATCH's `attack:'none'` can pounce):
- perceived **windup** in reach → **сбив**: a fast `INTERCEPT` jab (impact 0.2s vs PUNCH's 0.6s — a normal punch is far too slow to catch the foe's ~0.3s vuln window REACTIVELY, so only a sharp/low-delay reader lands it in time; a late read arrives during recovery = a normal trade — the «иногда, растёт с counter» behaviour falls out of the timing). Landing inside the foe's `windupVulnUntil` triggers the existing `staggerInterrupt`.
- perceived **recovery / stagger** in reach → **контра**: a brief visible coil (`gatherPose`, a deeper sink than the CATCH stance, held `read.gatherSec` ≈ 0.16s) then a PUNCH lunge — reads as «собрался → выпад», a MOMENT, not a silent exchange. The coil plants the body (own `gather` pose-mode, no nav / no attack until it lands).
- **CATCH/HOLD boost** the read chances (`read.catchBoost` / `read.holdBoost`) — Засада is the dedicated waiter that reads + pounces hardest, so a held CATCH wait converts into a strike.

**Picker link.** The perceived phase rides into the intention picker as `foe.phase` (`intentions.js spinalScore`): a perceived opening/windup adds a counter-scaled `readPounce` to CATCH's score, so a sharp reader ADOPTS CATCH to set up the pounce (the body's `tryReadReaction` does the actual strike).

**Dev readout.** New `RD` line in `.arena-readout` (alongside STA/CHG/INT/MDL): each fighter's perceived foe phase + the last reaction, e.g. `RD  P WINDUP!SBIV  ·  O NEUTRAL` — so the read skill is visible in isolation (high-counter shows reads + SBIV/CONTRA far more than low-counter). Getters: `getReadPhase()`, `getReadAction()`.

**Files:** `src/data/combatBalance.js` (new `read` block + `readDelaySec`/`readMissChance`/`readFalseChance`/`readWindupReactChance`/`readOpenReactChance` exports), `src/scene/buildFighter.js` (`getFoePhase` option, `getActionPhase`, read subsystem + `tryReadReaction`, `INTERCEPT` clip, `gatherPose`, `launchStrike` factored out of `decideAttack`, AI-loop hook, read/gather state reset on eliminate + new bout, return getters), `src/data/intentions.js` (`foe.phase` + CATCH `readPounce`), `src/scene/ArenaScene.vue` (`getFoePhase` both ways, `RD` readout). Build green.

### Space preview scene — `/play/space` (Ground Select → SPACE door)

Visual-only 3D preview of the future "Space" battlefield (no combat / server / match). Built by the PVE recipe as its own copy — protected files (`buildArena`, `buildFighter`, `arenaTextures`, `arenaPresence`, `intentions`, `intentionMotion`) and `PveScene.vue` / `home.css` are only read/called, never edited.

**Flow:** `… → Ground Select (/play/ground) → SPACE door → /play/space` (ARENA door still → `/play`). The SPACE door was previously locked (aria-disabled, SOON badge, no-op); it is now a live matte-chrome door — SOON badge removed, `pickSpace()` → `router.push('/play/space')`, ENTER-slot label `t.space.doorEnter` ("Enter →"). Ground Select discipline preserved: ARENA stays the SELECT screen's only pink/glow; the SPACE door stays matte chrome (no pink/glow) — the leader glow lives INSIDE the scene.

**New files:**
- `src/scene/SpaceScene.vue` — the scene. Instances `buildArena` as the central slab with the rift suppressed exactly like PVE (rift-glow opacity 0, sparks off, outline Lines hidden, dark seam-filler), then lays a big **hex FIELD** around it (`buildHexField`: dark base plane + tiled `makeHexGridTexture` lattice, arena tone, flush with the slab top) so it reads as a huge arena, not a bigger plate. Own copies of the PVE lamp room-fill + flat-dark backdrop dome. **14** fighters (`SPACE_ROSTER_COUNT`, CONFIG range 12–16) via `buildFighter`, core colours cycled (ONSLAUGHT #FF3344 / RAIDER #FFA526 / BULWARK #2ED6B0 / AMBUSH #9461FF) as body light/rim, spread across the field off the central seam, HP plates suppressed (sprite scan). One **leader** (index 0) carries the scene's SINGLE glow — a pink (`--hex-primary`) beacon (`buildLeaderMarker`: additive ground disc + soft light column, follows the roving leader, gentle pulse); every other body is unglowed. OrbitControls like PVE/Home but pulled back + higher (`CAM` block) with polar clamps so it never dips under the field. Emits `window.__hexSpaceReady` + `hexlash:space-ready` on the first frame. All knobs in the `FIELD` / `CONFIG` / `LEADER` / `CAM` / `LAMPS` blocks up top.
- `src/scene/spaceWander.js` — `createSpaceWanderDirector`, the multi-agent wander director copied from `pveWander.js` and tuned for 14 agents on a big field (bigger legs / winding / inter-fighter clearance; big overlapping personal zones → members range across the field without clumping in the centre). Drives the EXISTING locomotion via `getFoePos` lures; reduced-motion ⇒ inert (all idle).
- `src/views-v2/SpaceView.vue` — the 2D wrapper (mirrors `PveView`): `SpaceScene` + the shared `.hs-strip` (no brand — SHOP + cabinet chip on `.hs-chrome`, tokens mirrored on `.space-root`), ← Back → `/play/ground`, no FIGHT. Plus the honest **"mode coming soon"** note (`t.space.previewNote`) — a bottom-centre matte card carrying the SOON mark moved off the door; no second glow, no pink.

**Wiring edits:** `src/router/index.js` — new `V2Space` route (`path: 'space'`, `meta.scene3d`) under `/play`, + `beforeEach` fade mapped to `hexlash:space-ready`. `src/main.js` — readiness latch adds the `V2Space → hexlash:space-ready / __hexSpaceReady` case. `src/locales/en.js` — new `space: { previewNote, doorEnter }`. `src/views-v2/GroundSelectView.vue` — SPACE door unlocked (see Flow above).

**Discipline:** one pink on screen = the leader beacon (`--hex-primary`); one glow = the leader beacon; core colours ride the bodies as light/rim (never a second accent glow); calm dark room; scene covered by the load layer (`meta.scene3d` + `hexlash:space-ready`) so it never assembles on-screen.

---

## Единый экран загрузки — честная готовность

Один экран на все тяжёлые входы, и он снимается только когда сцена действительно собрана. До этого было два разных прикрытия, и оба снимались по **первому нарисованному кадру** — то есть в том же синхронном тике, где сцена достраивалась: браузер ещё не показал кадр, Three.js в этот момент лениво компилирует шейдеры и заливает текстуры, а `ResizeObserver` со своим стартовым вызовом переподгонял канвас **уже после** того, как прикрытие ушло. Игрок смотрел, как сцена встаёт на место.

### Договор готовности

Сцена объявляет свои этапы сборки, отмечает их по факту, и только после **трёх подряд устоявшихся кадров** заявляет готовность. Экран снимается когда сошлись все три условия: сцена готова, прогресс дошёл до 100, прошло минимальное время показа.

```js
const load = beginSceneLoad(['renderer', 'slab', 'atmosphere', 'fighter', 'stages']);
load.stage('slab');       // этап реально закончен
load.frame();             // в цикле, ПОСЛЕ renderer.render()
load.unsettle();          // сцена сдвинула картинку под собой (resize) → счёт заново
load.dispose();           // onBeforeUnmount
```

`beginSceneLoad` неявно добавляет первым этапом сам монтаж (чанк маршрута скачан и компонент исполняется) — на холодном кэше это самый крупный шаг перехода, и не считать его значило оставлять полосу у нуля, а потом дёргать её скачком.

### Файлы

| Файл | Роль |
|---|---|
| `src/services/sceneLoading.js` | **новый** — контроллер: этапы, устоявшиеся кадры, прогресс, тайминги, страховка. Единственный, кто пишет `progress` |
| `src/views-v2/SceneLoadingOverlay.vue` | экран, внутриигровая половина (был `SceneTransitionOverlay.vue`) |
| `index.html` `#hx-load` | экран, первая половина — красится до бандла, поэтому не может быть компонентом; та же картинка, `HexlashLoader.set / done / handoff` |
| `src/services/sceneTransition.js` | **удалён** — двухрежимный предшественник |

Две поверхности рендерят **одно** состояние (`loadingState`) и одну картинку. Какая из них наверху — вопрос момента, не вида.

### Прогресс

Этапы занимают полосу `0..95`, последние 5 — стабилизация. Число: **никогда не уменьшается** (единственный писатель — `bump`), **никогда не забегает** вперёд непройденного этапа (подползает на 90% зазора и ждёт), **никогда не показывает 100 пока экран не уходит** (100 и снятие — один момент). На холодном старте контроллер получает `from: 64` — вехи бутстрапа (бандл разобран → стор готов → приложение смонтировано) настоящие, и начинать счёт сцены с нуля под ними — ровно то, как полоса едет назад.

### Тайминги — `LOADING` в `sceneLoading.js`

`MIN_SHOW_MS: 600` (иначе на быстром соединении мигает) · `FADE_OUT_MS: 200` (обе поверхности; в `index.html` продублировано в CSS) · `SAFETY_MS: 15000` (снимает принудительно и **пишет в консоль, какой этап не завершился**).

### Что прикрывается и что нет

Прикрывается любой переход, строящий тяжёлую сцену: `/play/home`, `/play/mode`, `/play/pve`, `/play/space`, `/play/arena` — и первый вход, и внутриигровые, одинаково. **Не** прикрывается: 2D-маршруты, панели поверх сцены, и **кинематографический пролёт дом⇄режимы** (`isHomeStageHop`) — это одна живая сцена и поставленная анимация, а не загрузка.

**Лёгкое затемнение (`beginFade` / `mode: 'fade'`) удалено целиком.** `HomeView.stage-dim` — **не** оно и остаётся: это 130 мс, прикрывающие мгновенную перестановку камеры дом⇄режимы **только** при системной «уменьшить движение» (там камера не летит, а телепортируется). Снять его — оставить переход вообще ничем не прикрытым.

### Крайние случаи

Первый вход — одна непрерывная поверхность (сплеш держит до готовности; `handoff()` снимает его в тот же тик, если overlay всё же поднимается, чтобы два одинаковых экрана не наложились). Уход с экрана в процессе — `cancelLoading()`, а токен сессии осиротит хэндл брошенной сцены, так что её опоздавший сигнал не погасит следующую загрузку. Быстрые перескоки — каждая загрузка со своего нуля. Фон вкладки — цикл отрисовки остановлен, счётчик кадров просто замирает; страховка считается по реальным часам, но снятие **ждёт возвращения**, чтобы экран не исчез на глазах. Падение сцены — `app.config.errorHandler` → `noteSceneError()` снимает экран сразу, не дожидаясь страховки. Подсказка «поверни телефон» гасится на `loadingState.active`. Ввод под экраном заблокирован (`pointer-events: auto`).

### Известные расхождения (осознанные, по решению владельца)

- **`/dev/lab`** (`FighterLabScene`) — 3D-маршрут без `meta.scene3d`, без сигнала и без прикрытия: при заходе видно сборку. Dev-only, прямым URL, ни из одного меню не залинкован. Оставлен как есть.
- Экран загрузки — задокументированное исключение из дисциплины «один розовый / одно свечение»: он живёт по брендбуку.
- Текст на 100% (`ARENA READY`) общий для всех направлений — унаследован от сплеша, зеркалится в overlay ради «одного экрана»; для зала FORGE читается неточно. Косметика, отдельной задачей.

---
## Знак Hexlash — второй знак, растровый (05.09.2026)

Знак заменён на всех поверхностях одной правкой. Эталон —
`docs/design-handoff/hexlash_mark/` (README = правила, INTEGRATION.md = как
подключено и где разошлись).

**Новый знак:** шестиугольник с толстым контуром, рассечённый диагональными
росчерками. Белый `#F6F4F6` и **один** розовый росчерк `#FF0069`.

**Правило бренда изменено владельцем 05.09.2026:** белый плюс единственный
акцент — фирменный розовый ровно на одном росчерке. Прежнее правило о цвете знака
отменено. Свечения, градиентов и объёма в знаке по-прежнему нет. Логотип как был
вне дизайн-системы, так и остался: правила «одно свечение на экран», «цвета ядер
владеют бойцами и товаром» и «розовый владеет интерфейсом и деньгами» на него не
распространяются, потому что розовый здесь — часть знака, а не сигнал интерфейса.

**Охранное поле — не менее половины ширины знака с каждой стороны** (Документ А
§0.3). Конкурирующее число из README эталона снято владельцем 05.09.2026: число
живёт в одном месте, остальные на него ссылаются.

### Знак поставлен картинками — это промежуточный шаг

Вектора, геометрии и `.svg` в пакете нет: прислали PNG. Позже знак переведут в
вектор и заменят ещё раз, поэтому подключение сделано так, чтобы та замена была
одной операцией — положить файлы в `docs/design-handoff/hexlash_mark/brand/`,
прогнать `node scripts/sync-brand-icons.mjs`. Разметку восьми экранов трогать не
придётся. Своей отрисовки знака в коде нет и заводить её нельзя.

Следствия растрового шага: знак **не красится из кода**; на плотных экранах
требует файлов увеличенного размера, иначе мылит; для печати и крупных
материалов вектор придётся отрисовать отдельно.

### Одна точка входа

`src/components/brand/hexlashMark.js` — единственное место, знающее имена файлов.
**Две** отрисовки с одним порогом: `бокс >= 48px — полная, < 48px — упрощённая`.
Плотность экрана закрыта через `srcset`; где бокс резиновый (загрузка, 404),
компоненту передают `sizes` руками — самое большое значение по всем переломам.

Знак **несёт свои цвета в файле** и больше не наследует `color`. Предыдущий был
одноцветным `fill="currentColor"`; все семь объявлений `color`, стоявших только
ради знака, сняты. Светлого фона у знака сейчас нигде нет — появится, понадобится
отдельный файл инверсной версии.

### Поверхности (8)

Панель игры 48px · экран загрузки, обе половины · шапка и подвал лендинга (48px)
· вход · сброс пароля · 404 — плюс иконки вкладки и приложения и картинка превью
ссылки.

**С правки 05.09.2026 (ветка `claude/brand-mark-surfaces`) все боксы приложения
стоят от 48 и выше, то есть везде идёт ПОЛНАЯ отрисовка.** Упрощённая в
приложении не участвует нигде; файлы `mark-micro-*` остаются как страховка на
случай, если какой-то бокс когда-нибудь опустят.

⚠️ **Бокс знака нельзя менять, не проверив границу 48px** — отрисовка должна
меняться вместе с ним.

**Слово HEXLASH рядом со знаком стоит не везде.** Есть: экран загрузки (обе
половины), шапка лендинга (скрывается ≤1240px), подвал лендинга, 404. Нет: панель
игры, вход, сброс пароля. Правила, где ставить связку, а где один знак, в системе
нет сознательно — решается по месту (решение владельца 05.09.2026).

### Минимальный размер и слово

Знак не ставится в боксе меньше **32 точек**. Исключение одно — иконка вкладки,
где размер задаёт браузер: для 16 точек есть отдельная отрисовка, доведённая
вручную (знак во всю плитку без поля, контраст усилен). Это предел растрового
подхода, а не образец.

**Слово HEXLASH никогда не подключается картинкой** — везде живой текст в вёрстке.
Правило без исключений: `lockup-*.png` в пакете эталона справочные, в код не идут.

**Правила, где ставить связку, а где один знак, в системе нет — сознательно**
(решение владельца 05.09.2026). Решается по месту. Измеренное ограничение: слово
читается уверенно от 70 точек высоты, читается на 48, на 32 буквы слипаются.

**Пропорции горизонтальной связки — решение владельца 05.09.2026**, не вывод из
канона. Отношения сняты с отгруженной связки в подвале лендинга и пересчитываются
от фактического бокса знака: **знак 44 · слово 24 · зазор 14**. Типографика слова
— Saira Condensed 900, заглавными, разрядка `ls-title` (Документ А §0.6).
Выравнивание — по оптическому центру, не по коробке.

Эталон реализации — связка в шапке лендинга (`.lp .nav-logo` в `landing.css`):
всё выведено от переменной `--mark` через `calc(var(--mark) * 24 / 44)` и
`calc(var(--mark) * 14 / 44)`, связка масштабируется целиком.

⚠️ Числа связки в панели игры (40 / 24 / 12) **не подмешивать** — берётся
подвальная пара целиком. И ⚠️ связка в подвале расходится по типографике:
`.lp .footer-word` стоит насыщенностью 800 и разрядкой `.16em`, семейство не
объявлено. Пропорции при этом совпадают. Привести подвал к шапке — отдельная
задача.

### Иконки

`node scripts/sync-brand-icons.mjs` раскладывает файлы эталона в `public/` и
пересобирает `favicon.ico` (теперь 16 + 32 + 48). Скрипт **копирует и не рисует**
— именно самостоятельная отрисовка когда-то развела иконку вкладки со знаком в
игре.

### Удалено этой правкой

- `public/favicon.svg` — был собран руками и линковался первым, поэтому вкладка
  осталась бы со старым знаком. Вектора в пакете нет, а рисовать SVG руками —
  заводить вторую копию знака.
- Прежний пакет эталона (SVG, `geometry.json`, лист приёмки) — заменён целиком.

### Известные расхождения

- ~~**Мелкий бокс на плотном экране.**~~ Закрыто 05.09.2026: дизайнер прислал
  упрощённую отрисовку в 96/128/192, обе отрисовки теперь дотягивают до тройной
  плотности на всех боксах приложения. Правка свелась к списку файлов — разметка,
  порог 48px и логика переключения не тронуты.
- ⚠️ **Лесенка собрана из двух разных форм.** Опись 05.09.2026 нашла три формы
  там, где должно быть две: файлы 32/64 и файлы 96/128/192 отличаются рисунком
  росчерков и стоят в одном списке размеров. Раньше это было видно на панели игры
  (бокс 40) и в подвале лендинга (бокс 44) — разный знак на обычном и на плотном
  экране. Оба бокса подняты до 48, поэтому **на экранах дефект больше не
  проявляется**, но сам он никуда не делся: первый же бокс ниже 48 вернёт его.
  Лечится пересборкой лесенки из одного исходного файла, код не трогается. Разбор
  и таблица — `docs/design-handoff/hexlash_mark/README.md`.
- ⚠️ **Иконка вкладки на 32 точки показывает упрощённый знак, на 48 — полный.**
  Полной плитки 32 в пакете нет, а инструмента для пережатия картинок в проекте
  нет тоже, поэтому кодом это не чинится: нужна перерисованная `favicon-32.png`
  от дизайнера. После неё замена — одна команда `sync-brand-icons.mjs`. Размер 48
  сейчас лежит только внутри сборного `favicon.ico`, отдельной ссылки на него в
  `index.html` нет.
- **Пометка о пороге 48px стоит не у всех мест подключения** — она есть у четырёх
  из восьми (панель игры, подвал лендинга, вход, сброс пароля).
- **Знак на экране загрузки грузится файлом**, а не вписан в разметку (предыдущий
  одноцветный путь красился мгновенно). Взамен стоит `<link rel="preload">`.
- **Файлы в `public/` без версии в имени** — после подмены браузер может показать
  старую картинку из кэша. Так же ведут себя иконки, поведение не новое.

---

## Откуда взялись правила знака — историческая справка

⚠️ Раздел не описывает ни один знак. Он сохраняет **причину**, по которой
нынешние правила устроены именно так. Описание действующего знака — секцией выше
и в `docs/design-handoff/hexlash_mark/`.

**Летом в проде было два разных логотипа одновременно:** в игре и на лендинге
один рисунок, в иконке вкладки и приложения — совсем другой, а og-картинка была
третьим вариантом. Причина: знак в приложении заменили, а зашитые от руки
координаты в генераторе иконок (`scripts/gen-favicons.mjs`) — нет. Каждое место
по отдельности выглядело правдоподобно, и расхождение прожило месяцы, хотя никто
не правил ни один из логотипов нарочно.

Отсюда три действующих правила:

1. **Одна точка входа** — `hexlashMark.js`, единственное место, знающее имена
   файлов. Восемь экранов знают только размер своего бокса.
2. **Скрипт иконок только копирует, никогда не рисует.** Самостоятельная
   отрисовка по переписанным координатам и создала ту вилку.
3. **Все размеры производятся из одного исходного файла**, а лесенка сверяется
   глазами при каждой поставке. Это правило добавлено 05.09.2026 после того, как
   опись нашла в лесенке две разные формы.

Тогда же экран загрузки, 404 и авторизация знак **получили** — до этого там был
только текст. Композиция (вертикальный лок-ап, знак сверху, слово снизу, связка от
одной переменной `--word`) сохранена и на нынешнем знаке.

Удалено той правкой: `src/assets/images/logo-512.png`, `scripts/gen-favicons.mjs`,
`LogoMark` из `components/landing/icons.js`.

---
## Страница-дека для инвесторов — `/deckinvestors` (06.09.2026)

Отдельная **статическая** страница, а не экран приложения: Vue там не
запускается, роутер игры про этот адрес не знает, бандл не грузится. Решение от
03.09.2026: сайт игры — сложная сборка, вживление деки внутрь означало бы
взаимный риск.

Содержимое — презентация на 17 разделов (выгрузка Claude Design). В разделе
«04 · ЖИВОЙ БОЙ» стоит окно с **настоящей ареной прода**.

| Файл | Роль |
|---|---|
| `public/deckinvestors/index.html` | вся страница целиком: разметка, стили, мета, свой сценарий |
| `vercel.json` | исключение из перехвата всех адресов |
| `src/scene/ArenaScene.vue` | режим показа арены (см. ниже) |
| `src/router/index.js` | пропуск режима показа мимо проверки выбранного ядра |

### Адрес держится двумя вещами, и обе легко снести нечаянно

1. Форма «папка со своим `index.html`» — её одинаково понимают и Vercel, и
   nginx (`try_files $uri $uri/` в `nginx.prod.conf`).
2. Исключение в `vercel.json`: там перехват **всех** адресов
   `/(.*)` → `/index.html` для одностраничного приложения, и без исключения он
   съел бы страницу. ⚠️ Комментариев в `vercel.json` быть не может (строгий
   JSON) — объяснение живёт в шапке самой страницы и здесь.

### Выгрузка распакована заранее — и почему это важно

Claude Design отдаёт самораспаковывающийся файл: 211 КБ React плюс свой рантайм
собирают страницу в браузере при каждом открытии. В репозитории лежит
**распакованный результат**. Это разом закрыло три беды исходника:

- светлую вспышку перед сборкой (у выгрузки фон `#faf9f5` до старта);
- пустую страницу без JavaScript;
- шестиугольники, заданные как `sc-camel-view-box` вместо `viewBox` — чинить их
  умел только тот самый рантайм, без него они не рисуются вовсе.

Плюс убрана четвёртая, уже своя: меню в шапке не пряталось на телефоне
(встроенный стиль `display:flex` сильнее правила `@media`) и уезжало за край.

Собственного сценария у страницы ~6 КБ: проявление разделов на прокрутке,
полоска прочитанного и окно с ареной. Без сценария страница читается целиком —
за это отвечает блок `<noscript>` в шапке.

⚠️ **Повторная выгрузка от дизайнера не кладётся в репозиторий как есть.** Её
надо снова распаковать и пересобрать — иначе вернутся все четыре беды.

Значения вписаны числами (как на экране загрузки `#hx-load`): страница живёт вне
сборки, `tokens.css` туда не приезжает. Источник правды прежний.

Знак — те же файлы `public/brand/*`, разметка написана руками (`hexlashMark.js`
это Vue-модуль и сюда не грузится). Бокс 48 точек в шапке и в подвале — порог,
то есть полная отрисовка; ради этого высота шапки поднята с 58 до 76 точек.
⚠️ Опустить бокс ниже 48 — показать другой рисунок знака.

Закрыта от поиска мета-запретом плюс `X-Robots-Tag` в `vercel.json`. В
`robots.txt` путь **не упоминается намеренно**: строки запрета там публичны и
работают как указатель. Ссылок на страницу нет ниоткуда.

### Режим показа арены — `?showcase=1`

Одна арена, одна подача. При признаке показа: ядра назначены жёстко (ONSLAUGHT
против AMBUSH), служебная панель и кнопка `DEV` **не создаются**, проверка
выбранного ядра пропускается, бой стартует сам и доигрывается до конца.
**Без признака арена ведёт себя ровно как раньше** — служебная панель на обычной
арене по-прежнему видна по умолчанию, это отдельный блокер демо.

Арена сообщает окну на странице о себе сама, через `postMessage`:
`ready` (экран загрузки ушёл, бой пошёл) и `end` (бой доигран). Окно по этим
сообщениям меняет подпись состояния и показывает «ЕЩЁ РАЗ».

⚠️ **ПРАВИЛО, КОТОРОЕ НЕЛЬЗЯ НАРУШАТЬ** (решение владельца 06.09.2026, записано
и рядом с самим признаком в `ArenaScene.vue`, и в роутере). Признак управляет
**только подачей** — что видно и что запускается. Он **никогда** не управляет
включением думающего мозга модели, ограничителем расходов и вообще ничем, что
стоит денег: признак приходит из адресной строки, значит его подставит кто
угодно и нажжёт вызовов. Включение модели делается **одноразовым пропуском с
сервера**, отдельной задачей.

### Окно боя на странице — условия, которые нельзя нарушать

- ⚠️ **Окно всегда шире, чем выше** (16:9, на узких экранах 16:10). В
  вертикальном окне внутри арены вылезает подсказка «поверни телефон»: она
  смотрит на пропорции своего окна, а не устройства.
- ⚠️ **`pointer-events: none` на окне.** Без этого палец, ведущий по окну,
  крутит камеру арены и страница на телефоне не прокручивается (замерено).
- Арена не грузится, пока не нажали кнопку: сцена тяжёлая, автозагрузка
  подвесила бы вкладку.
- Экран загрузки один — арены, внутри окна. Своего окно не показывает, только
  подпись состояния в шапке карточки.

### Известные расхождения

- Заголовок `X-Robots-Tag` добавлен только на пути Vercel. `nginx.prod.conf` не
  тронут: какой из двух путей отдаёт прод, из репозитория однозначно не следует.
  Мета-запрет внутри страницы работает на обоих путях.
- Лист стилей презентации несёт золотой акцент `#FFB21D` (ссылки при наведении,
  часть свечений) — второго акцента в дизайн-системе нет. Витрина живёт по своим
  правилам, как и лендинг; приводить к системе — отдельное решение владельца.
- Камера арены рассчитана на полный экран, поэтому в окне сцена читается
  мелковато. Правка камеры в разрешение 06.09 не входила.

---

## SPAR — бой-настройка. Вход предметом из зала FORGE (24.09.2026)

Экран, на котором игрок берёт СВОЕГО бойца и **собирает соперника**: ядро,
кристаллы, заготовки «ЗЕРКАЛО» (точная копия) и «ЧИСТЫЙ» (то же ядро без единого
кристалла). Настоящего боя тут пока нет — кнопка «В БОЙ» открывает заглушку.

Заведён макетом на служебном адресе (ТЗ 24.09.2026 v1 → v2), в игру встроен ТЗ
v3. **Служебного адреса `/dev/spar` больше нет** — двух копий одного экрана в
проекте не держим.

| Что | Где |
|---|---|
| Экран | `src/views-v2/SparView.vue` (маршрут `/play/spar`, `meta.scene3d`) |
| Сцена | `src/scene/SparScene.vue` — двое друг против друга, камера неподвижна |
| Предмет в зале | `buildSparStand()` в `src/scene/forgeProps.js` |
| Место предмета | `buildForgeProps` в `src/scene/PveScene.vue` |
| Уход по нажатию | `onPress` в `src/views-v2/PveView.vue` |

### Предмет SPAR в зале — из чего собран

Низкий шестигранный постамент (`CylinderGeometry`, тон `MATERIALS.pedestal`) и
**тот самый упрощённый силуэт**, что стоит в кольце учеников гексарха —
`figureGeometry()` из `modePlates.js`. Ничего не рисуется заново: то же уже
делает `gateEmblems.js`. Матовый, без свечения, без лица.

- ⚠️ **Силуэт заметно ниже настоящего бойца** (`figScale: 0.80` против роста
  ~1.28). В полный рост он читался вторым бойцом, а не предметом.
- ⚠️ **Стоит на переднем краю, впереди наковальни** (`z + 0.3`). На одной с ней
  глубине он ложился на неё и читался одним спутанным предметом. Двигая —
  проверять снимком в обеих раскладках: камера зала диагональная, и
  разъехавшиеся по миру предметы на экране сходятся.
- Отклик на нажатие — общий для всех предметов зала (розовая лужица под
  предметом). ⚠️ Раньше зал вёл её только у надписей-переходов, а планшет,
  наковальня и полка на нажатие молчали; со встраиванием SPAR это стало видно и
  исправлено одним списком — **новой манеры для одного предмета не заводится**.
  У самого SPAR лужицу игрок фактически не видит: экран загрузки встаёт в первые
  же миллисекунды.

### Возврат в зал — память камеры

Зал пересобирается на каждом входе, поэтому поза камеры снимается на уходе и
возвращается на входе (`saveCamMemo` / `restoreCamMemo` в `PveScene.vue`).

- ⚠️ **Память живёт в обычном `<script>`, а не в `<script setup>`.** Всё, что
  объявлено в `script setup`, компилятор кладёт внутрь `setup()` — то есть
  заводит заново на каждом входе, и память молча теряется.
- ⚠️ **Восстановление повторяется в первом проходе `ResizeObserver`.** Тот
  дёргает обработчик один раз сразу после монтажа, и его `applyCamera` затирал
  восстановленную позу домашней.
- Вместе с позой запоминается ширина плиты: она растёт ступенями под размер
  ростера, и поза с узкой плиты на широкой смотрит мимо.

### Известные расхождения

- **Шестая тяжёлая сцена.** `.claude/skills/hexlash-3d` держит порог «пять, и
  шестую не заводить». SPAR — шаг за него, сделан по прямому заданию. Сцена
  лёгкая: два тела, неподвижная камера, ни ходьбы, ни разлома.
- **Пустой ростер:** сцена пуста (обе стороны гасятся), а сообщение «БОЙЦОВ НЕТ»
  лежит внутри панели — на телефоне её надо открыть. Выход при этом виден всегда:
  «← В ЗАЛ» в шапке.
- **Внешность соперника** — запертая кнопка со словом «скоро». Ни окна, ни
  заготовки за ней нет.
