...
| Path | View | Auth |
|------|------|------|
| `/auth/login` `/auth/signup` | AuthLayoutView > AuthSelectorView (Эпик 9 auth-redesign — provider-selector with state machine: provider/more/email + referral overlay; replaces 1b LoginView/SignupView) | No |
...
| Auth Forms | `auth/AuthSelectorView.vue` + `components/auth/*` (7 files, Эпик 9 auth-redesign — replaces 1b LoginView/SignupView). Provider-selector with local state machine, 4 screens: A (4 providers + referral CTA) → B (more options: email/farcaster/discord) → D (email form, mode-aware: handle on login, handle+email on signup, password) and C (referral overlay, Teleport-to-body). Tabs Login↔Signup with `router.replace` + `watch(route.path)` guarded sync (browser back/forward). 5 "coming soon" toasts via `InfoMessageModel.withoutButton`. Email field collected in UI but NOT in BE payload (TODO(auth-email) — pending BE extension). Referral code → `localStorage['hexlash_referral_code']` — `masterService.register` auto-reads + clears on success. Touch targets ≥44px throughout (Phase 3.5 closed sub-44 secondary controls). 4 corner-marks decorative (desktop only). `.auth-selector-*` / `.auth-tabs-*` / `.provider-btn-*` / `.email-form-*` / `.more-options-*` / `.referral-overlay-*` BEM-light scoped classes. Implementation report: `docs/auth-redesign-implementation-report.md`. |
