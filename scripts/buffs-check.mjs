...
console.log('\n── ВЕДРО: движение (и доказательство, что удары не трогает) ');
{
  ...
  // ⚠️ ЗАМЕР ПЕРЕВЁРНУТ (решение владельца 22.09.2026). Раньше здесь мерили
  //    ПРИБАВКУ к частоте ударов, и она не набиралась: ведро укорачивало ПАУЗУ
  //    между ударами, а сам удар длился сколько длился — 1.3 с из цикла в 1.7 с.
  //    Даже при множителе 10 (пауза почти в ноль) частота росла на десятую
  //    часть, а на рабочем 1.3 тонула в шуме боя. Обещание «бьёт чаще» было
  //    невыполнимым без ускорения самой анимации удара, то есть без правки
  //    сердца моторики.
  //
  //    Рычаг снят. Теперь здесь доказывается ОБРАТНОЕ: частота ударов не растёт
  //    ни на рабочем множителе, ни на заведомо крайнем.
  //
  //    ⚠️ СРАВНЕНИЕ НЕ СТРОГОЕ, И ЭТО НЕ НЕБРЕЖНОСТЬ. Ведро по-прежнему меняет
  //       ХОД, а ход меняет дистанцию, а дистанция — когда боец решит бить.
  //       Поэтому бой с ведром и без него не совпадает шаг в шаг даже теперь, и
  //       требовать равенства до удара значило бы ловить шум. Решающая величина
  //       здесь — ПОТОЛОК: при множителе 10 (пауза уходила почти в ноль) раньше
  //       частота росла на десятую часть, теперь расти нечему.
  ...
  const ceil = huge.rate / plain.rate - 1;
  ok(gain <= 0.02, 'ведро НЕ ускоряет удары',
     `(${plain.rate.toFixed(3)} → ${fast.rate.toFixed(3)} удара в секунду, ${gain >= 0 ? '+' : ''}${(gain * 100).toFixed(0)}%)`);
  ok(ceil <= 0.02, 'рычага частоты больше нет даже на крайнем множителе',
     `(множитель 10: ${plain.rate.toFixed(3)} → ${huge.rate.toFixed(3)} — было +10%)`);
  console.log('  · ведро ускоряет только передвижение, ровно как написано на карточке в магазине.');
