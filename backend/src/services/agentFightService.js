...
function distributeXpByBranch(roundLog, totalXp) {
  ...
  // Ensure at least 1 XP per used branch
...
/**
 * Run a PvE Training fight for an agent.
...
async function runPveTraining(agentId, userId) {
  ...
  // Set fighting status
  await prisma.agent.update({ where: { id: agentId }, data: { status: 'fighting' } });
  ...
  try {
    ...
    // Stats update fields
    const statsUpdate = { totalFights: { increment: 1 }, lastFightAt: new Date(), status: 'idle' };
    ...
    // Atomic transaction: update agent + progression + create fight log
