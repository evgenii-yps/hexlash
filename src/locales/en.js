export default {
  ...
  ascension: {
    // ⚠️ КОНФЛИКТ С ПРАВИЛОМ ПРОЕКТА, ОСТАВЛЕН НА РЕШЕНИЕ ВЛАДЕЛЬЦА.
    //    Выше (шапка секции landing) записано: внутренние слова — HOUSE,
    //    HEXARCH, ASCENSION, TEMPER, DOCTRINE — во внешний текст не идут. А ТЗ
    //    от 24.09.2026 называет этим словом и предмет зала, и экран: «В зале
    //    FORGE появляется объект/кнопка ASCENSION».
    //    Слово оставлено так, как его назвал владелец, и НЕ подменено молча.
    //    Если правило сильнее — менять надо ровно в двух местах: `title` ниже
    //    и `FORGE_PROPS.ascension.label` в src/scene/forgeProps.js. Адрес
    //    /play/ascension при этом можно не трогать: его игрок не читает.
    title: "ASCENSION",
    back: "\u2190 FORGE",
    // Экран выбора
