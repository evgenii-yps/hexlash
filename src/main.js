...
// ⚠️ ПРИЗНАК СВОБОДНОЙ КАМЕРЫ ПОДНИМАЕТСЯ ЗДЕСЬ, А НЕ В СЦЕНЕ БОЯ, И ЭТО НЕ
//    ПЕРЕСТРАХОВКА. Модуль сцены грузится лениво, уже после перехода, и к тому
//    моменту адрес сменился — признака в нём нет. Читать его надо на загрузке
//    страницы, до всякой навигации: тогда ?freecam=1 можно поставить где угодно
//    (хоть на доме) и дойти до боя обычной дорогой. Та же причина и тот же
//    приём, что у соседней строки.
import '@/services/freeCamMode.js';
import {
    VAlert,
    VBtn,
    VCard,
