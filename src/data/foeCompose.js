 ...
 *
 * ГРАНИ ПОЛУЧАЮТ ТОЛЬКО СОЮЗНИКИ. Чужая сторона выходит без граней вовсе.
 *
 * ⚠️ ПРАВИЛО ИСПРАВЛЕНО 17.09.2026, И ЭТО НЕ ВКУСОВЩИНА. Сначала грани игрока
 *    раздавались всем семерым — «сторона равна стороне», как в SQUAD. Замер
 *    показал, что так получается наоборот: боец с пятью гранями выиграл 3 рейда
 *    из 10, а боец без граней — 12 из 20. Причина арифметическая. Игрок один из
 *    ЧЕТВЕРЫХ, и его прокачка размазывается по команде; босс на своей стороне
 *    ОДИН, те же пять граней достаются ему целиком, да ещё поверх тройной брони.
 *    Прокачка игрока усиливала врага сильнее, чем команду игрока, — то есть
 *    работала против того, кто её делал, и против главного обещания игры
 *    «обученный боец сильнее необученного».
 *
 *    Союзникам грани оставлены: они и есть та половина, которой прокачка игрока
 *    должна доставаться. Охране и боссу — ноль: чужая сторона в рейде держится
 *    бронёй и числом, а не тем, сколько игрок успел вложить в своего бойца.
 *
 ...
 *
 * @param {number} litCount сколько граней зажжено у бойца игрока — столько же
 *                          достанется КАЖДОМУ СОЮЗНИКУ. Чужой стороне: ноль
...
export function composeRaid({ litCount = 0, rnd = Math.random } = {}) {
  ...
  // Союзники — с гранями игрока: это та половина плиты, которой его прокачка
  // и должна доставаться.
  const allies = [];
  ...
  const boss = one({
    litCount: 0,                 // чужая сторона граней не получает — см. шапку
  ...
  const guards = [];
  for (let i = 0; i < R.guards; i++) guards.push(one({ litCount: 0 }));
