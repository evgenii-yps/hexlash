...
export const COMBAT_BALANCE = {
  ...
  chain: {
    ...
    // Замер 16.09, 20 забегов на 640×360 (по 10 на бойца без граней и с пятью):
    // при -20 / -10 / 0 целиком не прошёл НИ ОДИН. По раундам доля побед была
    // 80/60% · 50/17% · 0/0% — третий раунд не взяли ни разу за пять попыток.
    // Причина в лоб: в третий раунд боец входит на 28-62 здоровья против свежей
    // сотни, а поправки у соперника там уже нет.
    //
    // Все три сдвинуты вниз, лестница сохранена: каждый следующий соперник
    // по-прежнему сильнее предыдущего, просто все трое слабее, чем были.
    roundBonus: [-0.35, -0.3, -0.25],
    ...
    // Пауза между замиранием боя и панелью следующего раунда (с). Нужна, чтобы
    // игрок успел увидеть, чем бой кончился: своего показа исхода у арены нет.
    panelDelaySec: 1.5,
  },
