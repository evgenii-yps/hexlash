...
export const COMBAT_BALANCE = {
  ...
  openField: {
    ...
    // --- СУЖЕНИЕ ПОЛЯ. Граница сходится к середине и подрезает хвост матча.
    //
    //     ⚠️ ВСЕ ЧИСЛА СУЖЕНИЯ — ВРЕМЕННЫЕ. Они подобраны на бумаге, а не
    //        замером, и подстраиваются ПОСЛЕ ПЕРВОЙ ПРОВЕРКИ НА ТЕЛЕФОНЕ. Лежат
    //        одним блоком намеренно: владелец крутит ощущение здесь и больше
    //        нигде.
    //
    //     ЧТО ЭТО ВООБЩЕ ТАКОЕ. Круг поверх поля, радиус которого со временем
    //     уменьшается. Тело снаружи круга каждый кадр получает сдвиг к середине —
    //     не больше, чем нужно, чтобы вернуть его на границу. Граница НЕ РАНИТ:
    //     расчёт урона этим блоком не тронут ни одним числом.
    //
    //     СЕРЕДИНА КРУГА — СЕРЕДИНА ПОЛЯ, И ЗА КОРОНОЙ ОН НЕ ХОДИТ. Игрок должен
    //     заранее видеть, куда сходится поле; лидер по ходу матча меняется, и
    //     круг начал бы дёргаться вслед за сменой. Столкновение двух тяг (корона
    //     тянет к лидеру, круг — к середине) — это содержание финала, а не изъян.
    shrink: {
      // КОГДА ГРАНИЦА ТРОГАЕТСЯ С МЕСТА, секунда матча. До неё поле целое.
      startSec: 90,
      // СКОЛЬКО ОНА СХОДИТСЯ, секунд. После этого стоит на конечном радиусе и
      // больше не двигается — матч доигрывается в свалке.
      durationSec: 60,
      // КОНЕЧНЫЙ РАДИУС, мировые единицы. АБСОЛЮТНОЕ число, одно на все три
      // раскладки, а не доля поля.
      //
      // ⚠️ ПОЧЕМУ АБСОЛЮТНОЕ. Размеры раскладок расходятся втрое (кольцо выхода
      //    SOLO 28.6 · DUO 16.2 · QUAD 9.9). Конечный круг отвечает на вопрос
      //    «сколько тел помещается в свалке», а не «какая доля поля осталась», —
      //    а тела во всех трёх раскладках одинаковые, и их двадцать.
      finalRadius: 4.0,
      // ОБРАТНЫЙ ОТСЧЁТ, секунд до начала. Заодно это и миг, когда граница
      // ПОЯВЛЯЕТСЯ на полу: отсчёт и контур — одно и то же предупреждение, и
      // показывать их врозь значило бы сказать одно и то же дважды.
      warnSec: 10,
      // --- ВИД ГРАНИЦЫ. Контур на полу, не стена: игрок видит и куда сходится
      //     поле, и тех, кто ещё снаружи. Своего света у неё нет — матовый хром
      //     берётся из общего набора материалов (MATERIALS.rim, кромка плиты:
      //     сужение и есть кромка, только движущаяся), и второго свечения на
      //     экране не появляется.
      lineWidth: 0.16,      // толщина контура в мировых единицах, постоянная
      lineY: 0.014,         // над плитой, чтобы не тонуть в ней
      lineOpacity: 0.5,
      // Дыхание контура: доля, на которую гуляет прозрачность, и период.
      // ⚠️ Гасится при системной настройке «уменьшить движение» — но САМО
      //    СУЖЕНИЕ не гасится ничем: это правило матча, а не украшение.
      pulseAmp: 0.18,
      pulseSec: 2.2,
      // --- УКРЫТИЯ ЗА ГРАНИЦЕЙ УХОДЯТ В ПОЛ.
      //
      // ⚠️ УХОДИТ БЛОК, КОТОРЫЙ ПЕРЕСТАЛ ПОМЕЩАТЬСЯ ЦЕЛИКОМ, а не тот, чью
      //    середину граница уже прошла. Иначе блок какое-то время торчал бы
      //    поперёк границы, и два правила тянули бы тело в разные стороны:
      //    выталкивание из блока — наружу, граница — внутрь.
      sinkSec: 0.5,         // сколько блок опускается; мгновенный провал читается как сбой
    },
  },
