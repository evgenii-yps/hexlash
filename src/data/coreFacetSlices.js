...
export function crystalSlots(f, n) {
  ...
  /* ⚠️ Когда кристаллов трое, каждая подпись идёт СВОЕЙ СТРОКОЙ, лесенкой.
     Иначе не сходится: три двухсловных названия в ряд на телефоне либо
     смыкаются, либо ужимаются до нечитаемого кегля (мерил: 8 точек). На своей
     строке подпись не встречается с соседями вовсе, и кегль режется только
     очень длинным именем. На двух кристаллах места хватает в один ряд. */
  const stagger = n >= 3;
  const rowDrop = f.lenU * 0.19;
  /* Сколько ширины достаётся подписи. Своя строка — можно брать всю грань. */
  const labW = stagger ? 2 * f.halfW : stepV * 0.68;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const lab = at(labU - (stagger ? rowDrop * i : 0), v);
    out.push({
      i,
      ...bevelQuad(q, 0.74),
      ...
      /* Ширина места под подпись: по ней кегль ужимается, чтобы соседние
         названия не смыкались. */
      slotW: +labW.toFixed(3),
      /* ⚠️ Через ряд название идёт ОДНОЙ строкой. Перенос на две строки
         опускал вторую строку ровно в соседний ряд, и подписи налезали друг
         на друга (поймано рендером на грани с тремя кристаллами). */
      oneLine: stagger,
    });
  }
  return out;
