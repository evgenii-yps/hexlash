<!-- OpenFieldPanels — надписи ОТКРЫТОГО ПОЛЯ: счётчик живых сторон поверх боя и
     ...
     ⚠️ СЧЁТЧИК — БЛОК, А НЕ СТРОКА, и это уже дважды окупилось. Заведён он был
     `<dl>` с одной строкой; второй стала строка лидера, третьей — обратный
     отсчёт сужения поля. Обе вставились одной строкой разметки и ничего не
     подвинули. Четвёртая встанет так же.
...
<template>
  ...
  <div v-if="onField" class="of-hud" aria-live="polite">
    <dl class="of-rows">
      ...
      <!-- СУЖЕНИЕ ПОЛЯ. Третья строка того же списка и в той же одёжке: новых
           стилей здесь нет ни одного. Стоит ПОСЛЕДНЕЙ намеренно — живёт она
           десять секунд, и появись она выше, счётчик сторон и лидер на эти
           десять секунд уехали бы вниз и обратно.

           Розового нет: розовый в этом режиме принадлежит короне. Граница —
           матовая и на полу, её показание тут тоже матовое. -->
      <div v-if="closingIn" class="of-row">
        <dt>{{ t.openField.closingIn }}</dt>
        <dd>{{ closingIn }}</dd>
      </div>
    </dl>
  </div>
...
// Обратный отсчёт до сужения — только пока поле на экране. Сцена кладёт сюда
// число лишь в последние секунды перед началом; во всё остальное время здесь
// пусто, и строки нет вовсе.
const closingIn = computed(() => (onField.value ? openFieldState.closingIn : null));
