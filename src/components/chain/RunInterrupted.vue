<!-- RunInterrupted — «RUN INTERRUPTED». Короткое сообщение о том, что прошлый
     ЗАБЕГ или ТУРНИР не доигран: обновили страницу, закрыли вкладку, ушли
     кнопкой «назад».

     ОДНО СООБЩЕНИЕ НА ДВА РЕЖИМА, И ЭТО НАМЕРЕННО. Игроку важен один и тот же
     факт — начатое не доиграно, — и говорить об этом двумя разными голосами
     незачем. Отметки при этом РАЗНЫЕ: прерванный забег и прерванный турнир
     лежат в памяти вкладки порознь, иначе один выдавал бы себя за другой.
     Файл назывался ChainInterrupted.vue, пока режим был один.
...
import { takeStaleCollapse } from '@/services/collapseRun.js';
...
onMounted(() => {
  // Забрать факт брошенного забега ИЛИ турнира и стереть отметку. Второй раз
  // вернёт false — поэтому сообщение не может выйти дважды, даже если компонент
  // стоит в двух местах сразу.
  //
  // ⚠️ Забираем ОБЕ отметки, а не первую сработавшую: иначе брошенный турнир,
  //    случившийся после брошенного забега, остался бы лежать в памяти вкладки и
  //    выскочил бы сообщением на следующем заходе, уже ни к чему не относясь.
  const stale = takeStaleRun(props.skip) | takeStaleCollapse(props.skip);
  if (!stale) return;
  shown.value = true;
