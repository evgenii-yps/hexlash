// pveWander.js — the PVE "roster wander director". Same sanctioned external
// pattern as homeWander.js (which it must NOT touch — the home is live on prod),
// but generalised to an ARRAY of fighters: every club member strolls the plate on
// its OWN real footwork, pauses with varied waiting actions, and they keep clear of
// one another and of the central rift seam — all by DRIVING the EXISTING locomotion
// from outside (a moving "lure" through buildFighter's getFoePos + the dev gait),
// never a parallel animation system and never an edit to the protected combat files.
//
// Each agent gets a PERSONAL wander rectangle (computed by PveScene from its spawn,
// clamped to one side of the seam) so two members can never path into the same spot —
// independent zones + randomised initial delays/phases make the roster read as a
// living gym, not a synchronised drill. Reduced-motion ⇒ the whole director stays
// inert (every foePos null) → the bodies just idle, calm.
//
// All feel knobs live in CONFIG — tune on preview in one place.
import * as THREE from 'three';

const CONFIG = {
  // desync — each member waits a different beat before its first stroll, and never
  // in lock-step thereafter (independent phase machines + randomised pauses).
  initialDelayMin: 0.3,
  initialDelayMax: 2.6,

  // --- WALK (personal zones are small, so legs are shorter than the home's) ---
  minLegDist: 0.8,     // a new destination is at least this far from the last spot
  arcMax: 0.5,         // max perpendicular bow of the path (winding, not A→B straight)
  leadGap: 1.0,        // keep the lure this far AHEAD of the body so it WALKS, not circles
  safeLead: 0.85,      // hard floor on lure↔body distance (> the body's contact range)
  lureStep: 0.02,      // path-param advance per lead iteration
  leadStepMax: 12,     // cap lead iterations per frame
  shiverAmp: 0.04,     // tiny perpendicular shiver on the lure → the line breathes
  shiverFreq: 2.3,
  legMaxSec: 14,       // safety: never get stuck on one leg
  fastChance: 0.30,    // a leg is brisk this often, else a calm stroll
  midFlipChance: 0.24, // chance the gait speed changes once mid-leg (varied pace)
  ...
  // --- keeping members apart (the inter-fighter distance knob) ---
  agentClearance: 1.15, // destinations + lures stay this far from every OTHER member
};
...
export function createPveWanderDirector(opts = {}) {
  ...
  // machine, a bézier leg and a lure — a trimmed copy of the home director's body.
  ...
  // routes a destination / leads a lure onto a club-mate.
  ...
  function pickDestination(a, fromX, fromZ) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (d >= cfg.minLegDist) return { x, z };  // first far-enough, club-mate-clear spot
  ...
  // clear of the body AND of other members. The body navigates toward it on its own
  // footwork; never within its contact range, so the separation clamp never fires.
