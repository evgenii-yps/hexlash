...
export function createIslandDive({ camera, controls }) {
   ...
   *
   * ИСКЛЮЧЕНИЕ — `aim.dir`. У предмета может быть ЛИЦО: у кнопки старта это
   * крышка с надписью и ядром. К лицу подлетают спереди, иначе игрок упирается
   * в затылок предмета ровно в тот момент, когда его показывают крупно. Тогда
   * прицел приносит направление с собой, и оно сильнее взгляда игрока. Второго
   * режиссёра для этого не нужно: поездка та же, меняется одна строка — откуда
   * берётся луч.
   *
  ...
  function poseFor(aim) {
    ...
    // Луч «от прицела к камере», с зажатым наклоном. Прицел со своим лицом
    // (`dir`) задаёт его сам — см. пояснение выше.
    if (aim.dir) _off.copy(aim.dir);
    else _off.copy(camera.position).sub(aim.point);
