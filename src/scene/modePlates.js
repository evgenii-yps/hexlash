...
export const MODE_PLATES = {
  ...
  forge: {
    ...
    // ⚠️ ОТДЕЛЬНОГО ТОНА У ФИГУРЫ НА ПОСТАМЕНТЕ БОЛЬШЕ НЕТ, и это решение
    //    владельца 25.09.2026, проверенное глазами на экране. Тут стояли по
    //    очереди два: приподнятый `hexBody`, чтобы фигура «стояла наособицу», и
    //    подмес цвета её ядра, чтобы легенда узнавалась по телу. Оба сняты.
    //
    //    Подмес отклонён на живом экране: рядом с почти чёрными учениками и
    //    тёмной плитой даже тёмно-фиолетовое тело читается пятном и выпрыгивает
    //    из острова пластиковой игрушкой. Совпадения светлоты, которое я
    //    подгонял числом, для глаза не хватило — цвет виден сам по себе, а не
    //    только через яркость. Это ровно тот случай, от которого предостерегает
    //    правило системы: цвет ядра — это СВЕТ, а не краска.
    //
    //    Теперь тело в обоих состояниях ровно такое же тёмное, как ученики и
    //    плита (`body` выше), а кто именно поднят, говорит ТОЛЬКО сердце.
...
//
//      легенды нет  — тёмный безликий силуэт, БЕЗ сердца вовсе;
//      легенда есть — ТО ЖЕ САМОЕ ТЁМНОЕ ТЕЛО, но с ЗОЛОТЫМ СЕРДЦЕМ в груди —
//                     золото с подмесом цвета её ядра, по тем же правилам, что
//                     в зале и в обряде (ascensionRite.legendHue), одним местом
//                     на проект.
//
//    Тело в обоих состояниях одно и то же: тон учеников и плиты. Кольцо
//    учеников и постамент на месте всегда.
//
//    ⚠️ ЦВЕТ ЯДРА ЖИВЁТ ТОЛЬКО В СЕРДЦЕ. Пробовали отдать его и телу — владелец
//       отклонил на живом экране: рядом с почти чёрными учениками цветное тело
//       читается пятном, даже когда светлота подогнана числом. Разбор — в
//       настройках forge выше.
//
...
function buildForgeEmblem(o, topY, legend) {
  ...
  //
  // ТЕЛО ОДНО И ТО ЖЕ В ОБОИХ СОСТОЯНИЯХ — тёмное, в тоне учеников и плиты.
  // Ни цвет ядра, ни отдельный «приподнятый» тон сюда не идут (разбор — в
  // настройках выше). Различает состояния ТОЛЬКО сердце: его нет либо оно есть
  // и золотое.
  ...
  const tick = (t, lit, presence) => {
    ...
    hexMat.color.setHex(f.body).multiplyScalar(presence);
