...
export const GATE_PLATES = {
  ...
  // не сжимается вовсе. На пяти островах (3 + 2) при шаге 8.8 просвет оказался
  // 39 px при подписи 37 px, и подписи верхнего ряда легли на плиты нижнего
  // (замерено 844×390). Шаг 11.5 разводит ряды на 109 px и оставляет запас.
  // На отъезд камеры это не влияет: лёжа его задаёт ширина, а не глубина.
  //
  // Стоя глубина — главная ось, и растягивать её нельзя: дальний остров и так
  // идёт по нижней границе «не меньше 60% от ближнего» (замерено — 64% на паре).
  stepZ: 10.0,
  stepZPortrait: 10.0,
