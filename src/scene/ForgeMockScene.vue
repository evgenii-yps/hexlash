...
function buildRosterBodies(world) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Стартовое место — РОВНО ПО ВСЕЙ ширине зала, а не по кольцу и не по
    // середине полосы. Кольцо давало кучку в середине. Середина полосы не
    // помогла: у крайних полос её поджимает кромка, и десять точек садятся в
    // 5.9 единицы вместо 12.6 — десяти телам по 0.8 в ширину там тесно, они
    // перекрываются, и зал опять открывается комом. Раскладываем по шагу
    // ширина/число: на десяти это 1.26 на тело — уже просторно. По глубине
    // зигзаг, чтобы не выстроиться в шеренгу. Полосы при этом остаются: они
    // про то, где боец ХОДИТ, а не про то, где он встал.
    const step = (2 * halfW) / n;
...
function pickAt(ev) {
  ...
  // ⚠️ ВРЕМЕННОЕ РЕШЕНИЕ — снять при встраивании в настоящий зал.
  //
  // Груша работает как ПОЛ СВОЕГО ОСТРОВА: нажал по груше — камера летит к ней.
  // Правильное решение другое: расширить стартовую позу так, чтобы полоса пола
  // соседнего острова всегда была в кадре и по ней можно было попасть пальцем.
  // Здесь так сделать нельзя — стартовая поза этого макета защищена ТЗ v3 §5.2
  // (её дважды чинили и проверяли числами), а без неё по соседнему острову из
  // покоя НЕ ПОПАСТЬ ВООБЩЕ: замер 23.09.2026 — 58 точек по всему кадру, включая
  // полосу у самого края (x до 0.997), ни одного попадания. Остров виден краем
  // картинки, но тела под пальцем там нет.
  //
  // При встраивании в зал поза считается заново под другой набор предметов, и
  // тогда эта ветка УБИРАЕТСЯ, а груша возвращается к «ничего» — но только если
  // правильный путь заработает. Оставить оба нельзя.
  //
  // Когда камера УЖЕ над тренировочным островом, нажатие по груше не делает
  // ничего само собой: flyTo видит, что ехать некуда, и выходит.
  for (const bg of bags) {
    if (raycaster.intersectObject(bg.group, true).length) return { kind: 'island', which: 'training' };
  }
