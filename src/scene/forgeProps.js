...
// Силуэт бойца — ОБЩИЙ с кольцом учеников у гексарха (экран режимов). Своей
// копии здесь нет намеренно: одна геометрия на оба места.
import { figureGeometry } from './modePlates.js';
...
export const FORGE_PROPS = {
  ...
  // SPAR — низкий круглый постамент, на нём тёмная безликая фигура бойца.
  // Читается мгновенно: «твой будущий соперник», а собирать соперника и есть
  // суть SPAR. Рифмуется с дверью FORGE на экране режимов, где фигура тоже
  // стоит над основанием.
  //
  // ⚠️ ОБЕ ЧАСТИ УЖЕ ЕСТЬ В ПРОЕКТЕ, и ни одна не рисуется заново:
  //   · силуэт — figureGeometry() из modePlates.js, тот самый, из которого
  //     сделано кольцо учеников у гексарха. Один слитый буфер, один вызов
  //     отрисовки. Полная фигура (buildFighter) в зале стоит слишком дорого —
  //     кольцо у гексарха сделано силуэтами ровно по этой причине;
  //   · постамент — рецепт парящего постамента оттуда же: шестигранный диск.
  //     Здесь он НЕ парит и стоит на полу: по ТЗ постамент низкий.
  //
  ...
  ascension: {
    ...
    // ⚠️ СЛОВО СПОРИТ С ПРАВИЛОМ ПРОЕКТА — разбор и второе место, которое
    //    придётся менять вместе с этим, в src/locales/en.js, секция ascension.
    label: 'ASCENSION',
  ...
  spar: {
    ...
    // ⚠️ СИЛУЭТ ЗАМЕТНО НИЖЕ НАСТОЯЩЕГО БОЙЦА. Боец в зале — около 1.28 в тех
    //    же единицах; в рост с ним силуэт читался вторым бойцом, а не предметом
    //    (поймано первым же снимком зала). Ниже — и он читается фигуркой на
...
function propShell(label, labelWidth, labelY, labelZ,
  ...
  const pgeo = new THREE.PlaneGeometry(
    puddleW || FORGE_PROPS.press.radius * 2,
    puddleH || FORGE_PROPS.press.radius * 2,
  );
...
// Низкий шестигранный постамент, на нём тёмный безликий силуэт. Настройки и
// доводы — в FORGE_PROPS.spar выше.
//
...
export function buildSparStand() {
  ...
  // Силуэт. Тон — САМЫЙ ГЛУХОЙ из декоративных: фигура обязана читаться темнее
  // постамента под собой, иначе «безликий соперник» превращается в ещё одну
