...
function layout() {
  ...
  /* ⚠️ ПОД ШАПКУ ОТДАНА ПОЛОВИНА ЕЁ ВЫСОТЫ, А НЕ ВСЯ. Шапка прозрачная и
     ...
     нечитаемого. Половина оставляет знак и меню на своих местах, а блок
     заходит только под нижний, пустой край шапки. */
  availH = Math.max(120, vh - navH / 2 - 16);
...
function camTick() {
  ...
  const lift = aim * ((vh + navH / 2) / 2 - originY);
