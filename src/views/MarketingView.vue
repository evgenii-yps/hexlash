...
function layout() {
  ...
  /* Свободная высота — экран минус шапка: она прилипшая и накрыла бы верх
     блока. Высота шапки берётся с самого элемента: в стилях она объявлена
     выражением, числом её оттуда не прочитать. */
  const vh = window.innerHeight || 1;
  const nav = root.querySelector('.nav');
  navH = nav ? nav.offsetHeight : 0;
  /* ⚠️ ПОД ШАПКУ ОТДАНЫ ТРИ ЧЕТВЕРТИ ЕЁ ВЫСОТЫ, А НЕ ВСЯ. Шапка прозрачная и
     занимает до 180 точек (её высота выведена из размера знака) — отдать ей
     всю высоту значит отобрать у блоков четверть экрана и ужать текст до
     нечитаемого. Три четверти оставляют знак, меню и кнопку PLAY на своих
     местах, а блок заходит только под нижний, пустой край шапки.
     ⚠️ Было пополам — при этом верхняя строчка блока подходила к пунктам меню
     вплотную; с кнопкой PLAY в шапке это стало особенно заметно. */
  availH = Math.max(120, vh - navH * 0.75 - 16);
...
function camTick() {
  ...
  /* Середина не экрана, а свободной части под прилипшей шапкой — иначе шапка
     накрывает верх блока. */
  const lift = aim * ((vh + navH * 0.75) / 2 - originY);
  ...
  /* Идёт ли полёт прямо сейчас: окно полёта прилипло к экрану. От этого
     зависит одно — показывать ли PLAY в шапке. Класс ставим напрямую, а не
     через состояние: иначе Vue перерисовывал бы шапку каждый кадр. */
  const flying = r.top <= 0 && r.bottom >= vh;
  if (flying !== root.classList.contains('is-flying')) root.classList.toggle('is-flying', flying);
