    ...
    <main class="fx-page">
      ...
      <section id="s3" class="fx-sec">
        ...
        <p class="fx-note">
          ...
          боец — между ними. Ближний ряд (ROSTER · FORGE) — то, к чему ходят: он
          крупный и стоит ровно в той полосе экрана, которую сегодня занимает
          панель. Дальний (BUFFS) — задел, к нему не ходят: он уведён за бойца,
          меньше по перспективе и не спорит за ширину кадра.
        </p>
        <p class="fx-note">
          ...
          на 390 точках перестаёт читаться предметом. Почему предметы не встали
          все одним рядом перед бойцом: длинный ряд шире кадра телефона, камера
          отъезжает — и мелкими становятся сразу и предметы, и боец. Это проверено
          на кадрах, а не решено на словах.
        </p>
        <p class="fx-note">
          Предметов стало три вместо пяти: SHOP и кабинет вернулись в плоские
          кнопки. Освободившееся место отдано бойцу и разметке мест, а не
          растянуто между оставшимися предметами — растянуть их значило бы снова
          отогнать камеру.
        </p>
