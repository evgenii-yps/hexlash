    ...
    <div class="auth-form">
      ...
      <form class="auth-form__form" @submit.prevent="onSubmit" novalidate>
        ...
        <div class="auth-form__field">
          <label class="auth-form__label" for="signup-handle">
            Handle
        ...
        <!-- Password field -->
        <div class="auth-form__field">
          <label class="auth-form__label" for="signup-password">
            Password
        ...
        <div class="auth-form__field">
          <label class="auth-form__label" for="signup-confirm">
            Confirm password
            <span class="auth-form__required" v-if="errors.confirmPassword">{{ errors.confirmPassword }}</span>
          </label>
          <input
            id="signup-confirm"
        ...
        <p v-if="serverError" class="auth-form__server-error" role="alert">
          {{ serverError }}
        </p>
        ...
        <p class="auth-form__switch">
          ...
          <router-link to="/auth/login" class="auth-form__switch-link">Sign in</router-link>
        </p>
      </form>
    </div>
  </div>
...
const canSubmit = computed(() => {
  return (
    form.value.login.trim().length > 0 &&
...
function validate() {
  errors.value = { login: '', password: '', confirmPassword: '' };
  serverError.value = '';
  let valid = true;
  ...
  if (!form.value.confirmPassword) {
    ...
    errors.value.confirmPassword = 'must match';
    valid = false;
  }
...
async function onSubmit() {
  ...
  loading.value = true;
  try {
    // master/register action throws on error (unlike login which sets state).
    // On success: action calls router.push('/') → 1a beforeEnter cascades to /v2.
...
<style scoped>
/* Mirrors LoginView (C3) — same .auth-form-* taxonomy.
   When polishing, consider extracting to global hexlash-ui.css. Stream 1 polish. */
.auth-form-wrap {
  width: 100%;
  display: flex;
