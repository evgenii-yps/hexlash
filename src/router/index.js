...
const v2Routes = [
    {
        ...
        children: [
            ...
            {
                // SPAR — бой-настройка: игрок берёт своего бойца и СОБИРАЕТ ему
                // соперника, ядро и кристаллы. Вход один — предмет SPAR в зале
                // FORGE (ТЗ 24.09.2026, встраивание).
                //
                // ⚠️ СЛУЖЕБНОГО АДРЕСА БОЛЬШЕ НЕТ. Страница жила на /dev/spar,
                //    пока была макетом; двух копий одного экрана в проекте не
                //    держим, поэтому старый адрес снят вместе с переездом.
                //
                // meta.scene3d — вход тяжёлый (две фигуры, своя сцена), экран
                // загрузки держится до готовности, как у дома и зала.
                path: 'spar',
                name: 'V2Spar',
                meta: { scene3d: true },
                component: () => import('@/views-v2/SparView.vue'),
            },
            {
                // ПОСВЯЩЕНИЕ В ЛЕГЕНДУ. Одноразовое необратимое событие; вход
                // из зала FORGE, предметом ASCENSION.
                //
                // meta.scene3d — вход тяжёлый (своя сцена с телом бойца), и
                // общий экран загрузки обязан держаться до готовности.
                //
                // ⚠️ СВОЕГО СТОРОЖА У МАРШРУТА НЕТ НАМЕРЕННО. Единственное
                //    условие — «легенда ещё не поднята», и живёт оно в
                //    состоянии ростера, а не в адресе. Проверяет его сам экран
                //    и молча уходит в зал: сторож здесь читал бы то же самое
                //    хранилище, только раньше, чем оно поднято.
                path: 'ascension',
                name: 'V2Ascension',
                meta: { scene3d: true },
                component: () => import('@/views-v2/AscensionView.vue'),
            },
            {
